...
function getReminderCode(){
	...
	// public function getAssignedLeadsCount($employee_id = null) {
	// 	$this->db->select('id');
	// 	$this->db->from('lead_csv');
	...
	// 	// Agar date filter chahiye, uncomment karo
	// 	// $this->db->where('date', date('Y-m-d'));
    ...
    function get_menu_tree($parent_id) 
	{
	    ...
	    while($row=mysqli_fetch_array($res,MYSQLI_ASSOC)) 
		{
			   ...
			   $menu .= "<ul>".($row['menu_id'])."</ul>"; //call  recursively
	...
	public function leadsSummary($login_id, $role_id)
	{
		$data = [];
		...
		$this->db->from('lead_csv');
		$this->db->where('lead_status', 'Won');
		$data['won'] = $this->db->count_all_results();

		$this->db->from('lead_csv');
		$this->db->where('lead_status', 'Loss');
		$data['loss'] = $this->db->count_all_results();

		if ($data['total'] > 0) {
			$data['won_percent']  = round(($data['won'] / $data['total']) * 100, 2);
			$data['loss_percent'] = round(($data['loss'] / $data['total']) * 100, 2);
			$data['total_percent'] = 100; // total always 100%
		} else {
			$data['won_percent']  = 0;
			$data['loss_percent'] = 0;
	...
	public function getLatestLeads($limit = 50, $offset = 0)
	{
		$this->db->select('*');
		$this->db->from('lead_csv');
	...
	function getDesignations() { 
        ...
        // $result= $result->result_array();
        $departments = array(); 
        $departments[' '] = 'Select designation...'; 
        foreach($result as $r) { 
            $departments[$r['id']] = $r['designation']; 
        } 
        
        return $departments; 
    }

	public function getAssignedLeadsCount($employee_id) {
		$this->db->select('id');
		$this->db->from('lead_csv');
		$this->db->where('lead_assign', $employee_id);
		$count = $this->db->count_all_results();
		return $count;
	}

	public function getTotalLeadsCountForProgress() {
		return $this->db->count_all('lead_csv');
	}

	public function getWonLeadsCount($employee_id) {
		$this->db->select('id');
		$this->db->from('lead_csv');
		$this->db->where('lead_assign', $employee_id);
		$this->db->where('lead_status', 'Won');
    ...
    {
        $query = $this->db->query("
            SELECT DATE_FORMAT(date, '%b %Y') as month, COUNT(id) as total 
            FROM lead_csv 
	...
	{
		$this->db->select('lsa.activity_type, lsa.due_date, lc.leads_priority, lc.lead_code, lc.contact_person');
		$this->db->from('lead_schedule_activities lsa');
		$this->db->join('lead_csv lc', 'lc.id = lsa.lead_id', 'left');
		$this->db->order_by('lsa.due_date', 'DESC');
		$this->db->limit(4);
		$query = $this->db->get();
		return $query->result_array();
	}

	public function get_leads_overview() {
		$this->db->select('COUNT(*) as total');
		$total = $this->db->get('lead_csv')->row()->total;

		$this->db->select('COUNT(*) as won');
		$this->db->where('lead_status', 'Won');
		$won = $this->db->get('lead_csv')->row()->won;

		$this->db->select('COUNT(*) as loss');
		$this->db->where('lead_status', 'Loss');
		$loss = $this->db->get('lead_csv')->row()->loss;

		$this->db->select('COUNT(*) as qualify');
		$this->db->where('lead_status', 'Qualify');
		$qualify = $this->db->get('lead_csv')->row()->qualify;
