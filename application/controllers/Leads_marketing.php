...
Class Leads_marketing extends MY_Controller {
	...
	public function followups($id = NULL) 
	{
		...
		$this->template->load('layout/template','lead_marketing/marketing_followups',$data);
	}
	...
	public function insert_followup()
{
    ...
    $config['upload_path']   = './uploads/lead_followups/';
    ...
    if (empty($_FILES['photo']['name'])) {
        $this->session->set_flashdata('failed', 'Please upload a file, it is required!');
        redirect('/Leads_marketing/followups/' . $lead_id);
        return;
    }
	...
	// $followup_by = $this->session->userdata('login_id');
	$data['followup_by']  = $this->session->userdata['logged_in']['id'];

    // ✅ Insert followup
    $data = array(
        'lead_id'       => $lead_id,
        'answer'        => $answer,
        'file_path'     => $file_name, // required
    ...
    if ($result) {
        ...
        $this->db->where('id', $lead_id)->update('lead_marketing', $larray);

		$data['emp_id']  = $this->session->userdata['logged_in']['id'];
        // ✅ Insert history
        $leadhistory = array(
            'lead_id'    => $lead_id,
            'date'       => date('Y-m-d'),
        ...
        // ✅ Update last_update
        $this->db->where('id', $lead_id)->update('lead_marketing', [
            'last_update' => date('Y-m-d H:i:s'),
        ]);

        $this->session->set_flashdata('success', 'Follow Up Added Successfully !');
        redirect('/Leads_marketing/index/' . $lead_id);
    } else {
