...
public function index() {
		...
		if($this->input->get()) {
			...
			$data['employees'] = $this->employee->employeesList($conditions);
			...
			$this->template->load('layout/template','employees/employee_view',$data);
	}
	...
	public function add() 
	{
			...
			// if (isset($result['employee_description'])) :
	        //     $data['employee_description'] = $data['employee_description'];
	        // else:
			...
			// print_r($data['employees']);exit;
			$this->template->load('layout/template','employees/employees',$data);
	}
		...
		// print_r($photo);exit;
	        ...
	        if (isset($result['blood_group']) && $result['blood_group']) :
	            ...
	            $data['blood_group'] = '';
			     ...
			     if (isset($result['eligible_esi_pf']) && $result['eligible_esi_pf']) :
	            $data['eligible_esi_pf'] = $result['eligible_esi_pf'];
	       else:
	            $data['eligible_esi_pf'] = '';
	        endif;
			...
			if (isset($result['office_time_in']) && $result['office_time_in']) :
	            $data['office_time_in'] = $result['office_time_in'];
	       	else:
	            $data['office_time_in'] = '';
	        endif;

			if (isset($result['office_time_out']) && $result['office_time_out']) :
	            $data['office_time_out'] = $result['office_time_out'];
	       	else:
			...
			// print_r($data['departments']);exit;
			$this->template->load('layout/template','employees/employee_edit',$data);
	...
	public function add_new_employee() {
		...
		else 
		{
			...
			$eligible_esi_pf = $this->input->post('eligible_esi_pf') ? 1 : 0;
			...
			'eligible_esi_pf' => $eligible_esi_pf,
			...
			'office_time_in'=>$this->input->post('office_time_in'),
			'office_time_out'=>$this->input->post('office_time_out'),
	...
	public function editemployee($id) {
		...
		else 
		{
			...
			$data = array(
				...
				'designation_id'=>$this->input->post('designation_id'),
				'gender' =>$this->input->post('gender'),
				'eligible_esi_pf' =>$this->input->post('eligible_esi_pf'),
				// 'date_of_joining'=>date('Y-m-d',strtotime($this->input->post('doj'))),
				// 'dob'=>date('Y-m-d',strtotime($this->input->post('dob'))),
				'username' => $this->input->post('username'),
				'author_id' =>$this->input->post('author_id'),
