...
public function EmailVerify() {
	...
	public function otpVerify() {
		...
		$result=$this->login_database->verify_email($email);
		   ...
		   if (!empty($result) && $result[0]->forgot_code == $otp) {

        // ✅ Mark email as verified in database
			$update_data = array(
				'email_verified' => 1,
				'forgot_code' => NULL // optional: clear the OTP after verification
			);
        $this->login_database->updateOtp($update_data, $result[0]->id);
		//print_r($result);exit;
	...
	public function MyPasswordChangeView() {
		...
		if (isset($result['totalleads']) && $result['totalleads']) :
			$data['totalleads'] = $result['totalleads'];
		else:
			$data['totalleads'] = '';
		endif;

		if (isset($result['employee_description']) && $result['employee_description']) :
			$data['employee_description'] = $result['employee_description'];
		else:
			$data['employee_description'] = '';
		endif;

		if (isset($result['leave']) && $result['leave']) :
			$data['leave'] = $result['leave'];
		else:
			$data['leave'] = '';
		endif;

		if (isset($result['office_time_in']) && $result['office_time_in']) :
			$data['office_time_in'] = date('h:i A', strtotime($result['office_time_in']));
		else:
			$data['office_time_in'] = '';
		endif;

		if (isset($result['office_time_out']) && $result['office_time_out']) :
			$data['office_time_out'] = date('h:i A', strtotime($result['office_time_out']));
		else:
			$data['office_time_out'] = '';
		endif;
		...
		if (isset($result['author_email']) && $result['author_email']) :
			$data['author_email'] = $result['author_email'];
		else:
			$data['author_email'] = '';
		endif;
...
$result = $this->employee->getById($loginId);
$employee = (object)$result; // Convert to object for easy access

// Add employee object to view
$data['employee'] = $employee;

// Pass some individual fields if needed
$data['gender'] = $employee->gender ?? '';
$data['email'] = $employee->email ?? '';
$data['email_verified'] = $employee->email_verified ?? 0;
...
$data['designation_id'] = $this->session->userdata['logged_in']['designation_id'];
$data['totalleads'] = $this->login_database->totalLeads($loginId);
$data['leadsFollowups'] = $this->login_database->totalLeadsfollowups($loginId, $data['role_id']);
$data['assignLeads'] = $this->login_database->assignLeads($loginId, $data['role_id']);
...
$this->template->load('layout/template', 'new-password', $data);
	...
	public function get_monthly_leads()
	{
		$year = date('Y');
		...
		$query = $this->db->query("
			SELECT 
				...
				SUM(CASE WHEN lead_status='Loss' THEN 1 ELSE 0 END) as loss
			FROM lead_csv
		...
		for ($i = 11; $i >= 0; $i--) {
			...
			$data[] = [
				'month' => $monthName,
				'total' => isset($dbData[$key]) ? $dbData[$key]['total'] : 0,
				'won'   => isset($dbData[$key]) ? $dbData[$key]['won'] : 0,
				'loss'  => isset($dbData[$key]) ? $dbData[$key]['loss'] : 0,
			];
		}
	...
	public function get_leads_summary()
	{
		...
		$query = $this->db->query("
			SELECT 
				...
				SUM(CASE WHEN lead_status='Won' THEN 1 ELSE 0 END) as won,
				SUM(CASE WHEN lead_status='Loss' THEN 1 ELSE 0 END) as loss,
				SUM(CASE WHEN lead_status='Qualify' THEN 1 ELSE 0 END) as qualify
			FROM lead_csv
			WHERE YEAR(date) = ?
		", [$year]);
	...
	public function get_duplicate_details()
	{
		...
		$obj = $this->login_database->getCodedetails($lead_code);
		// return $obj;exit;
